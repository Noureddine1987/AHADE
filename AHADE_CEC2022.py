...
def main(dim):
    ...
    CEC2022_suite = [
        F12022(DimSize), F22022(DimSize), F32022(DimSize), F42022(DimSize),
        F52022(DimSize), F62022(DimSize), F72022(DimSize), F82022(DimSize),
        F92022(DimSize), F102022(DimSize), F112022(DimSize), F122022(DimSize)
    ]

    for i in range(len(CEC2022_suite)):
        FuncNum = i + 1
        print(f"Running Function {FuncNum} for {DimSize}D...")
        RunAHADE(CEC2022_suite[i])
...
if __name__ == "__main__":
    ...
    Dims = [10, 20] # As per HRCOA paper for CEC2022
    for dim in Dims:
        main(dim)